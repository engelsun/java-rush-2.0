...
public class Field {
    ...
    public void removeFullLines() {
        ...
        List<int[]> nonFullLines = new ArrayList();
        for (int i = 0; i < height; i++) {
            int countOfX = 0;
            for (int j = 0; j < width; j++) {
                countOfX += matrix[i][j];
            ...
            if (countOfX != width) {
                nonFullLines.add(matrix[i]);
            }
        }

        while (nonFullLines.size() < height) {
            nonFullLines.add(0, new int[width]);
        }

        matrix = nonFullLines.toArray(new int[height][width]);
    }
}
